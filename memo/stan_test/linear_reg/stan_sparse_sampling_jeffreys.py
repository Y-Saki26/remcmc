...
if __name__=="__main__":
    ...
    save_model_name = "sampling_model_jeffreys.bin"
    if os.path.exists(save_model_name):
        with open(save_model_name, "rb") as f:
            sm = pickle.load(f)
    else:
        sm = pystan.StanModel(file="sparse_linear_model_jeffreys.stan")
        with open(save_model_name, "wb") as f:
            pickle.dump(sm, f)
    print(sm)

    save_sample_name = "samples_data-3_jeffreys.bin"
    ...
    plt.savefig("hist_trace-3_jeffreys.png")
    plt.cla()
    plt.clf()
    plt.close()
